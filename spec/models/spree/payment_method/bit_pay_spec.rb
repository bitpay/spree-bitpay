...
describe Spree::PaymentMethod::BitPay do
  ...
  # it should have exactly one client?
  # we can attach a rails_bitpay_client and save the clients id
  #it { subject.association(:bit_pay_client).class.should  eq(ActiveRecord::Associations::HasOneAssociation) }
  ...
  context ".authenticate_with_bitpay" do
    it "should create a new BitPayClient with itself when attempting to pair" do
      ...
      random_string = ('0'..'z').to_a.shuffle.first((rand(10) + 10)).join
    ...
    it "should retrieve a pairing code" do
      bpclient = mock_model("BitPayClient")
      random_string = ('0'..'z').to_a.shuffle.first((rand(10) + 10)).join
      random_pairing_code = ('0'..'z').to_a.shuffle.first(7).join
      allow(bpclient).to receive(:save!)
    ...
    it "deletes pre-existing clients associated with payment method" do
      2.times{subject.authenticate_with_bitpay("your mom")}
